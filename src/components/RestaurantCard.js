...
const RestaurantCard = (props) => {
    ...
    return (
        <div className={shouldEnableCarousel ? "bg-gray-200 h-92 w-52 p-2 m-2 hover:cursor-pointer hover:scale-95 rounded-md" : "bg-gray-200 w-64 h-92 p-2 m-2 hover:cursor-pointer hover:scale-95 overflow-hidden rounded-md"}>
            ...
            <div className='overflow-hidden text-primary break-words font-bold leading-8 m-2 truncate'>{name}</div>
            <div className='flex space-x-1 m-2 leading-2 text-sm'>
                <div>
...
export const withOfferText = (RestaurantCard) => {
    return ({resData}) => {
        ...
        return (
            <div className='relative'>
                <label className='absolute font-extrabold  top-40 left-5 text-white z-10 hover:cursor-pointer hover:scale-85'>{offerDetailsInfo}</label>
